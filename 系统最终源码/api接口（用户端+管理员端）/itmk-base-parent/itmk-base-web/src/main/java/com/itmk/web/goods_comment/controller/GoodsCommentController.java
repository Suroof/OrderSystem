...
public class GoodsCommentController {
    ...
    public ResultVo edit(@PathVariable("commentId") Long commentId, @RequestBody GoodsComment goodsComment){
        GoodsComment comment = goodsCommentService.getById(commentId);
        if(comment == null){
            return ResultUtils.error("评论不存在");
        }
        BeanUtils.copyProperties(goodsComment, comment);
        goodsCommentService.updateById(comment);
        return ResultUtils.success("编辑成功");
    }

    //添加
    @PostMapping("/")
    public ResultVo add(@RequestBody GoodsComment goodsComment){
